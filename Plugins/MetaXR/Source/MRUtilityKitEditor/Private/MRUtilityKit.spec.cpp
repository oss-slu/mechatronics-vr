...
#else
BEGIN_DEFINE_SPEC(FMRUKSpec, TEXT("MR Utility Kit"), EAutomationTestFlags::ProductFilter | EAutomationTestFlags_ApplicationContextMask)
#endif
...
void FMRUKSpec::LoadSceneFromJson()
{
	BeforeEach([this]() {
		...
		// Ensure the data has been loaded into the anchor store
		WaitDiscoveryFinished();
	});
}
...
void FMRUKSpec::Define()
{
	...
	Describe(TEXT("Room Loading"), [this]() {
		...
		It(TEXT("Load empty room"), [this]() {
			...
			ToolkitSubsystem->LoadSceneFromJsonString(TEXT(R"({"CoordinateSystem": "Unreal", "Rooms": []})"));
			// Ensure the data has been loaded into the anchor store
			WaitDiscoveryFinished();
	...
	Describe(TEXT("Update room"), [this]() {
		...
		It(TEXT("Change nothing in room"), [this]() {
			...
			TestEqual(TEXT("No room updated"), O->RoomsUpdated.Num(), 0);
